...
@interface CBMainTabBarController ()<UITabBarControllerDelegate>
{
    UIView *_loadingView;
...
@implementation CBMainTabBarController
...
-(void)addViewControllers
{
    ...
    BuyLyVC * overseasVC =[[BuyLyVC alloc]init];
...
-(void)loginTimeOutGoToLoginViewController
{
    if (!self.presentedViewController) {
        ...
        [SVProgressHUD way_showInfoCanTouchWithStatus:[GQUser shareUser].tokenInfo.errorMsg dismissAfterInterval:1.5 onView:self.view];
    }
...
-(void)queryDomainInfo
{
    ...
    */
    ...
    NSString *paramsString = [NSString encryptedByGBKAES:[paramsDic JSONString]];
    [GQRequest cookBook_startWithDomainString:GQSerVerAPINameForAPIMain
                              apiName:@""
          ...
          if (request.resultIsOk) {
              ...
              if (domainUrl) {
                  ...
                  [DataCenter shareGlobalData].domainUrlString = domainString;
                  [DataCenter shareGlobalData].updateUrl = updateUrl;
                  [DataCenter shareGlobalData].lowVersion = lowVersion;
                  [DataCenter shareGlobalData].currentVersion = currentVersion;
...

{
    ...
    if ([viewController isKindOfClass:[UINavigationController class]]) {
        ...
        }else if ([rootViewController isKindOfClass:[BuyLyVC class]]){
            //购彩
