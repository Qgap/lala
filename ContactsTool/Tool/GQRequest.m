...
{
    GQRequest *sumRequest = [[GQRequest alloc]initWithApiName:apiName
                                                      params:params
    ...
    [sumRequest startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ...
        if ([request isKindOfClass:[GQRequest class]]) {
            GQRequest *sRequest = (GQRequest *)request;
    ...
    [sumRequest startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ...
        if ([request isKindOfClass:[GQRequest class]]) {
            ...
            if (sRequest.resultInfo) {
                if ([GQUser shareUser].tokenInfo.errorMsg.length>0) {
