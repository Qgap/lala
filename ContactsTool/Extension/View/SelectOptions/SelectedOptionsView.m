...
#import "SelectedOptionsCell.h"
#import "SelectedOptionsCollectionCell.h"

#define kReusedCellMark @"kReusedCellMark"

#define kHeightSelectedOptionsCell 44.0f


@interface SelectedOptionsView ()<UITableViewDataSource,UITableViewDelegate>
{
...
@property(nonatomic,copy)SelectedOptionsBlock selected;
...
@implementation SelectedOptionsView

-(void)awakeFromNib
{
    [super awakeFromNib];
    self.layer.opacity = 0;
    UINib * nib = [UINib nibWithNibName:@"SelectedOptionsCell" bundle:nil];
    [_tableView registerNib:nib forCellReuseIdentifier:kReusedCellMark];
    
    UINib * colNib = [UINib nibWithNibName:@"SelectedOptionsCollectionCell" bundle:nil];
...
{
    SelectedOptionsView *optionsView = [SelectedOptionsView createViewFromNib];
...
+(void)showWithOnView:(UIView *)superView
        ...
        selectedIndex:(NSInteger)selectedIndex
             selected:(SelectedOptionsBlock)selected
{
    [SelectedOptionsView showWithOnView:superView options:options selectedIndex:selectedIndex clearBgColor:NO selected:selected];
...
{
    BOOL used = YES;
    CGFloat tableViewHeight = optionsCount *kHeightSelectedOptionsCell;
    if (tableViewHeight>= supviewHeight *0.9) {
        used = NO;
        NSInteger row = optionsCount/4;
        row = optionsCount%4>0?row+1:row;
        CGFloat collectionViewHeight = row *kHeightSelectedOptionsCell;
...
{
    SelectedOptionsCollectionCell * cell = [collectionView dequeueReusableCellWithReuseIdentifier:kReusedCellMark forIndexPath:indexPath];
...
{
    CGSize size = CGSizeMake(self.width/4.0f, kHeightSelectedOptionsCell);
    return size;
}
...
{
    SelectedOptionsCell * cell = [tableView dequeueReusableCellWithIdentifier:kReusedCellMark];
