...
#import "LocalCodeLabel.h"
#import "CPTryPlayViewController.h"
@interface GQRegistViewController ()
{
    ...
    IBOutlet LocalCodeLabel *_codeLabel;
    IBOutlet UIScrollView *_scrollMainView;
}
@end

@implementation GQRegistViewController
...
-(void)queryRegistInfoIsLoading:(BOOL)isLoading
{
    ...
    [GQRequest cookBook_startWithDomainString:[DataCenter shareGlobalData].domainUrlString
                              apiName:GQSerVerAPINameForAPIRegistPreInfo
...
- (IBAction)buttonActions:(UIButton *)sender {
    ...
    switch (sender.tag) {
        ...
        {
            //法律声明
            GQWebViewController *toWebVC = [[GQWebViewController alloc] cookBook_WebWithURLString:[[NSString alloc]initWithString:[[DataCenter shareGlobalData].domainUrlString wayStringByAppendingPathComponent:GQSerVerAPINameForAPIRegLaw]]];
            toWebVC.title = @"协议";
...
-(void)queryRegist
{
    ...
    NSString *paramsString = [NSString encryptedByGBKAES:[paramsDic JSONString]];
    [GQRequest cookBook_startWithDomainString:[DataCenter shareGlobalData].domainUrlString
                              apiName:GQSerVerAPINameForAPIRegist
                               params:@{@"data":paramsString}
                         rquestMethod:YTKRequestMethodGET
           completionBlockWithSuccess:^(__kindof GQRequest *request) {
