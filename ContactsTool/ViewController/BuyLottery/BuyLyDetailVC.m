...
#import "GQBuyLotterySectionView.h"
#import "SelectedOptionsView.h"
#import "CookBookBuyCustomConfirmView.h"
#import "CBBuyKuaiCConfirmView.h"
#import "GQBuyLotteryFastItem.h"
#import "CPBetRecordVC.h"
#import "GQBuyLetterLotterySectionView.h"
#import "GQLotteryResultDetailWebVC.h"
#import "CPRechargeMainViewController.h"

@interface BuyLyDetailVC ()<CPBuyLotterySectionViewDelegate,CPBuyLetterLotterySectionViewDelegate,UIScrollViewDelegate>
{
    ...
    IBOutlet VoiceButton *_lianWeiBetButton;
    IBOutlet VoiceButton *_lianXiaoBetButton;
    ...
    GQLotteryShowResultView *_resultView;
    IBOutlet VoiceButton *_trendButton;
    
    GQRequest *_betRequest;
    GQRequest *_playInfoRequest;
}
...
- (void)viewDidLoad {
    ...
    [CBBuyLyManager shareManager].currentBuyLotteryType = resultType;
                        ...
                        errorMsg:(NSString **)errorMsg
{
    ...
    *isBet = NO;
    ...
    for (GQBuyLetterLotterySectionView *sectionView in _sectionViews) {
                             ...
                             errorMsg:(NSString **)errorMsg
{
    ...
    *isBet = NO;
    ...
    for (GQBuyLotterySectionView *sectionView in _sectionViews) {
...
-(void)setSelectedPlayKindIndex:(NSInteger)selectedPlayKindIndex
{
    ...
    [CBBuyLyManager shareManager].currentPlayKindDes = self.playKindNameList[_selectedPlayKindIndex];
}
    ...
    switch (sender.tag) {
        ...
        case 33:
        {
            ...
            if ([playKindName isEqualToString:@"合肖"]) {
                ...
                for (GQBuyLotteryFastItem *item in self.heXiaoBetLotteryInfos) {
                ...
                [CBBuyLyManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                [CBBuyKuaiCConfirmView showFastLotteryConfirmViewOnView:self.navigationController.view lotterys:@[betInfo] numberPeriods:[CBBuyLyManager shareManager].currentBetPeriod specailType:1 comfirm:^(BOOL isConfirm, NSString *value) {
                    if (isConfirm) {
                ...
                GQBuyLetterLotterySectionView *sectionView = _sectionViews[0];
                ...
                if ([self checkLianMaVerifyAndIsBet:&isBet errorMsg:&errorMsg]) {
                    if (isBet) {
                        if (self.lianMaBetInfos.count>0) {
                            [CBBuyLyManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                            [CBBuyKuaiCConfirmView showFastLotteryConfirmViewOnView:self.navigationController.view lotterys:self.lianMaBetInfos numberPeriods:[CBBuyLyManager shareManager].currentBetPeriod specailType:2 comfirm:^(BOOL isConfirm, NSString *value) {
                                if (isConfirm) {
                ...
                if ([self checkLianXiaoWeiVerifyAndIsBet:&isBet errorMsg:&errorMsg]) {
                    if (isBet) {
                        if (self.lianXiaoWeiBetInfos.count>0) {
                            [CBBuyLyManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                            [CBBuyKuaiCConfirmView showFastLotteryConfirmViewOnView:self.navigationController.view lotterys:self.lianXiaoWeiBetInfos numberPeriods:[CBBuyLyManager shareManager].currentBetPeriod specailType:2 comfirm:^(BOOL isConfirm, NSString *value) {
                                if (isConfirm) {
            ...
            if (self.betType == CPBuyLotteryBetFast) {
                ...
                [CBBuyLyManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                [CBBuyKuaiCConfirmView showFastLotteryConfirmViewOnView:self.navigationController.view lotterys:_betResultDic.allValues numberPeriods:[CBBuyLyManager shareManager].currentBetPeriod comfirm:^(BOOL isConfirm, NSString *value) {
                    if (isConfirm) {
                ...
                [CBBuyLyManager shareManager].currentBetPeriod = [self.playInfo DWStringForKey:@"period"];
                [CookBookBuyCustomConfirmView cookBook_showCustomLotteryConfirmViewOnView:self.navigationController.view lotterys:_betResultDic.allValues numberPeriods:[CBBuyLyManager shareManager].currentBetPeriod comfirm:^(BOOL isConfirm) {
...
- (IBAction)_titleButtonAction:(UIButton *)sender {
    ...
    NSInteger tag = 909;
    SelectedOptionsView *originOptionView = [self.view viewWithTag:tag];
    if (originOptionView) {
        [originOptionView dismiss];
        return;
    }
    
    SelectedOptionsView *optionView = [SelectedOptionsView showWithOnView:self.view
...
- (IBAction)sortAction:(UIButton *)sender {
    switch (sender.tag) {
        ...
        case 104:
        {
            //玩法说明
            NSString *gid = [_playInfo DWStringForKey:@"num"];
            NSString *urlString = [[DataCenter shareGlobalData].domainUrlString wayStringByAppendingPathComponent:[NSString stringWithFormat:@"/api/help?gid=%@",gid]];
...
-(void)reloadTopViewData
{
    ...
    _resultView = [[GQLotteryShowResultView alloc]initWithFrame:CGRectMake(5, 30, kScreenWidth - 2*5, 0)];
    CGFloat resultViewHeight = [GQLotteryShowResultView resultViewHeightByResult:result resultType:CPLotteryResultTypeByTypeString(resultType) isWaitOpenResult:lastOpen.length==0?YES:NO maxWidth:_resultView.width isList:NO];
...
-(void)reloadContentView
{
    @synchronized (self) {
        ...
        for (UIView *subView in _scrollView.subviews) {
            if ([subView isKindOfClass:[GQBuyLotterySectionView class]] || [subView isKindOfClass:[GQBuyLetterLotterySectionView class]]) {
                [subView removeFromSuperview];
            }
        }
...
//                _sortBetTypeView
                _sortBetTypeView.hidden = YES;
                GQBuyLetterLotterySectionView *sectionView = [[GQBuyLetterLotterySectionView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 0) selectMaxCount:11 sectionName:@"自选不中" delegate:self];
...
//                lianMaLotteryInfos
                ...
                for (int i = 0; i<self.lianMaLotteryInfos.count; i++) {
                    ...
                    GQBuyLetterLotterySectionView *sectionView = [[GQBuyLetterLotterySectionView alloc]initWithFrame:CGRectMake(0, 0, kScreenWidth, 0) selectMaxCount:maxCount selectMinCount:minCount sectionName:name delegate:self];
...
//                _topView.height = 160.0f-_sortBetTypeView.height;
                ...
                for (int i = 0; i<mPlayDetailList.count; i++) {
                    ...
                    GQBuyLotterySectionView *sectionView = [[GQBuyLotterySectionView alloc]initWithFrame:CGRectMake(0, 0,kScreenWidth, 0) buySectionInfo:info betType:CPBuyLotteryBetForLianXiaoWei delegate:self];
                    [_scrollView addSubview:sectionView];
                    lastView = sectionView;
                    [_sectionViews addObject:sectionView];
                }
            ...
            if (([playKindName rangeOfString:@"特码"].length>0 || [playKindName isEqualToString:@"正码"] || ([playKindName rangeOfString:@"正"].length>0&&[playKindName rangeOfString:@"特"].length>0))&& _betType == CPBuyLotteryBetCustom){
                ...
                for (int i = 0; i<playDetailList.count; i++) {
                    ...
                    GQBuyLotterySectionView *sectionView = [[GQBuyLotterySectionView alloc]initWithFrame:CGRectMake(0, 0,kScreenWidth, 0) buySectionInfo:info betType:CPBuyLotteryBetForLHCHorizontalViewStyle delegate:self];
                    [_scrollView addSubview:sectionView];
                    [_sectionViews addObject:sectionView];
                }
...
//        heXiaoLotteryInfo
        [GQUser shareUser].buyLotteryDetailHasNumberPan = [self isNumberPan];
        CPBuyLotteryBetType betType = specailType==3?CPBuyLotteryBetForHeXiao:_betType;
        UIView *lastView =_topView;
        for (int i = 0; i<playDetailList.count; i++) {
            NSDictionary *info = playDetailList[i];

            GQBuyLotterySectionView *sectionView = [[GQBuyLotterySectionView alloc]initWithFrame:CGRectMake(0, 0,kScreenWidth, 0) buySectionInfo:info betType:betType delegate:self];
            [_scrollView addSubview:sectionView];
            lastView = sectionView;
            [_sectionViews addObject:sectionView];
        }
                             ...
                             palyKindInfo:(NSDictionary *)playKindInfo
{
    @synchronized (self) {
        ...
        _playInfoRequest = [GQRequest cookBook_startRequestWithDomainString:[DataCenter shareGlobalData].domainUrlString
...
{    
    ...
    @synchronized (self) {
        ...
        NSString *gid = [_playInfo DWStringForKey:@"num"];
        NSString *roomId = [_playInfo DWStringForKey:@"roomId"];
        NSString *issue = [CBBuyLyManager shareManager].currentBetPeriod;
        ...
        NSString *paramsString = [NSString encryptedByGBKAES:[paramsDic JSONString]];
        _betRequest = [GQRequest cookBook_startRequestWithDomainString:[DataCenter shareGlobalData].domainUrlString
                                         apiName:GQSerVerAPINameForAPIBetSubmit
                                          params:@{@"data":paramsString}
                                    rquestMethod:YTKRequestMethodGET
                      completionBlockWithSuccess:^(__kindof GQRequest *request) {
               ...
               if (request.resultIsOk) {
                   ...
                   if ([request.requestDescription rangeOfString:@"余额不足"].length>0 && ![GQUser shareUser].isTryPlay) {
...
-(void)queryBalanceInfo
{
    ...
    [GQRequest cookBook_startWithDomainString:[DataCenter shareGlobalData].domainUrlString
                              apiName:GQSerVerAPINameForAPIUserAmount
                               params:@{@"data":paramsString}
                         rquestMethod:YTKRequestMethodGET
           completionBlockWithSuccess:^(__kindof GQRequest *request) {
