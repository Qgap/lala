...
#import "GQBuyLotteryCustomItem.h"

@interface GQBuyLotterySectionView ()
{
...
@implementation GQBuyLotterySectionView
...
-(CGFloat)itemHeight
{
    if (self.betType == CPBuyLotteryBetFast) {
        if ([CBBuyLyManager shareManager].currentBuyLotteryType == CPLotteryResultForK3) {
            if ([[CBBuyLyManager shareManager].currentPlayKindDes isEqualToString:@"点数"]) {
                return 50;
            }
            return 50+15;
        }
...
-(void)buildSection
{
    VoiceButton *clickBtn = [VoiceButton buttonWithType:UIButtonTypeCustom];
    ...
    for (int r=0 ; r<row; r++) {
        ...
        for (int l=0; l<line; l++) {
            ...
            if (self.betType == CPBuyLotteryBetForHeXiao) {
                ...
                [GQBuyLotteryFastItem cookBook_addBuyLotteryFastItemOnView:self.contentView withFrame:frame buyInfo:info detailLabelTextColor:kCOLOR_R_G_B_A(109, 109, 109,1) clickAction:^(GQBuyLotteryFastItem *item, NSDictionary *buyInfo, BOOL isSelected) {
                    if (isSelected) {
                        /*
                         */
                        if (self.heXiaoArray.count>=11) {
                            GQBuyLotteryFastItem *firstItem = [self.heXiaoArray objectAtIndex:0];
                ...
                [GQBuyLotteryFastItem cookBook_addBuyLotteryFastItemOnView:self.contentView withFrame:frame buyInfo:info clickAction:^(NSDictionary *buyInfo, BOOL isSelected) {
                ...
                [GQBuyLotteryFastItem cookBook_addBuyLotteryFastItemOnView:self.contentView withFrame:frame buyInfo:info infoIndex:infoIndex clickAction:^(NSString *infoIndex, BOOL isSelected) {
                ...
                [GQBuyLotteryCustomItem cookBook_addBuyLotteryCustomItemOnView:self.contentView withFrame:frame buyInfo:info clickAction:^(NSDictionary *buyInfo, NSString *value) {
...
-(void)buildLHCHorizontalViewStyleItems
{
    ...
    for (int i = 0; i<infos.count; i++) {
        ...
        [GQBuyLotteryCustomItem cookBook_addHorizontalBuyLotteryCustomItemOnView:self.contentView withFrame:frame buyInfo:info clickAction:^(NSDictionary *buyInfo, NSString *value) {
            [self.delegate cookBook_buyLotteryBetCustomBuyInfo:buyInfo sectionName:[self.buySectionInfo DWStringForKey:@"name"] value:value];
        }];
