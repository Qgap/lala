...
@interface GQBuyLotteryCustomItem ()<UITextFieldDelegate>
...
@implementation GQBuyLotteryCustomItem
                 ...
                 clickAction:(CPBuyLotteryCustomItemFinishedAction)action
{
    ...
    if (self) {
        ...
        NSString *haoString = [GQUser shareUser].buyLotteryDetailHasNumberPan?@"号":@"";
        haoString = ([CBBuyLyManager shareManager].currentBuyLotteryType == CPLotteryResultForK3 && [[CBBuyLyManager shareManager].currentPlayKindDes isEqualToString:@"点数"])?@"点":@"";
            ...
            if (![[CBBuyLyManager shareManager].currentPlayKindDes isEqualToString:@"点数"])
            {
                ...
                if ([playName isPureInt]) {
                    ...
                    for (NSInteger i = 0; i<playNameCount; i++) {
                        ...
                        imgView.image = [CBBuyLyManager k3BackgroundImageByNumber:number];
...
{
    GQBuyLotteryCustomItem *item = [[GQBuyLotteryCustomItem alloc]initWithFrame:frame buyInfo:buyInfo clickAction:acion];
...
+(void)cookBook_addHorizontalBuyLotteryCustomItemOnView:(UIView *)supView
                                   ...
                                   clickAction:(CPBuyLotteryCustomItemFinishedAction)acion
{
    GQBuyLotteryCustomItem *item = [[GQBuyLotteryCustomItem alloc]initWithHorizontalFrame:frame buyInfo:buyInfo clickAction:acion];
    [supView addSubview:item];
}
