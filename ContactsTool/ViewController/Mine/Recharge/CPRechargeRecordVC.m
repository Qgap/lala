...
- (IBAction)titleButtonAction:(id)sender {
    ...
    [SelectedOptionsAgoView showWithOnView:self.navigationController.view title:@"选择入款类型" options:_typeList selectedIndex:self.selectedType selected:^(NSInteger index) {
...
-(void)queryDetailInfoWithId:(NSString *)infoId
{
    ...
    [GQRequest cookBook_startWithDomainString:[DataCenter shareGlobalData].domainUrlString
                              apiName:GQSerVerAPINameForAPIUserRechargeDetail
...
-(void)queryDataForReferesh
{
    ...
    NSString *paramsString = [NSString encryptedByGBKAES:[paramsDic JSONString]];
    [GQRequest cookBook_startWithDomainString:[DataCenter shareGlobalData].domainUrlString
                              apiName:GQSerVerAPINameForAPIUserRechargeList
                               params:@{@"data":paramsString}
                         rquestMethod:YTKRequestMethodGET
           completionBlockWithSuccess:^(__kindof GQRequest *request) {
